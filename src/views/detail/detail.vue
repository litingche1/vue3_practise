...
watch(scrollTop, newVal => {
  if (newVal == domOffsetTop) clickState = false
  if (clickState) return
  const elList = Object.values(sectionEls.value)
  const valList = elList.map(el => el.offsetTop)
  let index = valList.length - 1
  for (let i = 0; i < elList.length; i++) {
    if (valList[i] > newVal + 44) {
      index = i - 1
      break
    }
  }
